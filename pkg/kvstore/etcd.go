...
func connectEtcdClient(ctx context.Context, config *client.Config, cfgPath string, errChan chan error, rateLimit int, opts *ExtraOptions) (BackendOperations, error) {
	...
	go func() {
		...
		for {
			...
			case event, ok := <-watcher.Events:
				...
				t, err := time.Parse(time.RFC3339, string(event.Value))
				if err != nil {
					log.WithError(err).WithField("value", event.Value).Warning("Unable to parse hearbeat value")
				} else {
					ec.RWMutex.Lock()
					// It is tempting to compare against
					// the heartbeat value stored in the
					// key. However, this would require the
					// time on all nodes to be
					// synchronized. Instead, assume current time
					// and print the heartbeat value in
					// debug messages for troubleshooting
					ec.lastHeartbeat = time.Now()
					ec.RWMutex.Unlock()
					log.WithField("heartbeat", t.String()).Debug("Received update notification of heartbeat")
				}
			}
		}
	...
	abort:
		watcher.Stop()
		errChan <- fmt.Errorf("heartbeat watcher stopped")
	}()
